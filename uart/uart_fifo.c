...
/**< RX FIFO buffer for storing data received on the UART until the application fetches them using app_uart_get(). */
...
uint32_t uartInit(nrf_uart_baudrate_t baud, bool parity){
  //buffers.rx_buf = rxBuf;                                                              
  //buffers.rx_buf_size = sizeof (rxBuf);                                                     
  buffers.tx_buf = txBuf;                                                              
  buffers.tx_buf_size = sizeof (txBuf);                                                     
  //uint32_t errCode = app_uart_init(P_COMM_PARAMS, &buffers, EVT_HANDLER, APP_IRQ_PRIORITY_LOWEST);
...
*/
    ...
    errCode = nrf_drv_uart_init(&app_uart_inst, &config, uartEventhandler);
    if(errCode != NRF_SUCCESS){
      return errCode;
    }
      ...
      if (errCode == NRF_SUCCESS) {
        ...
        if (!nrf_drv_uart_tx_in_progress(&app_uart_inst)) {
            ...
            if (app_fifo_get(&txFifo, tx_buffer) == NRF_SUCCESS) {
                errCode = nrf_drv_uart_tx(&app_uart_inst, tx_buffer, 1);
            }
        }
      }else {
        break;
